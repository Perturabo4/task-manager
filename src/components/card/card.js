...
const Priority = ({ priority }) => {
  ...
  return (
    <span style={{ fontSize: "0.8rem" }}>
      Приоритет:
      <br />
      <span style={{ color: color[priority], fontSize: "1.2rem" }}>
        {text[priority]}
      </span>
    </span>
  );
...
const Card = ({ auth, task, updateTask }) => {
  ...
  return (
    <div className={`card ${classes.join(" ")}`}>
      <div className="card-shadow">{inProgres && <Spinner />}</div>
      {done && (
        <CheckBox
          onChange={() => updateTask(id, null, ["done", false], auth.userId)}
        />
      )}
      ...
      <div className="card-content">
        <p>{text}</p>
        <span className="date-add">{dateAdd}</span>
      </div>
